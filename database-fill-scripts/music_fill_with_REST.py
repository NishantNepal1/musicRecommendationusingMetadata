...
def preprocess_playlist_1(playlist, spotify_conn):  # playlist here is csv with uri
    ...
    for j in range(limitcount + 2):
        songleft = l - j * limit
        if songleft > limit:
            Features = spotify_conn.audio_features(uri_of_playlist[j * limit:j * limit + limit])
            ...
            Features = spotify_conn.audio_features(uri_of_playlist[j * limit:j * limit + songleft])
...
def generate_data_new_way(spotify_connection, playlist):
    ...
    for i in range(l):
        result = spotify_connection.track(uri_of_playlist[i],)
        list_of_songs.append([{"song": result["name"]}])
        artist_name.append({"artist": result["artists"][0]["name"]})  # get the artist_name
        popularity.append({"popularity": result['popularity']})  # get the popularity of the song
        bar.update(i)
    ...
    for j in range(limitcount + 2):
        songleft = l - j * limit
        Features = spotify_connection.audio_features(uri_of_playlist[j * limit:j * limit + limit])
        if songleft > limit:
            for i in range(limit):
                list_of_songs[j * limit + i][0].update(artist_name[i])
                list_of_songs[j * limit + i][0].update(popularity[i])
...
def generate_data(spotify_connection, json_data):
    ...
    for json_song in json_data:
        ...
        del features_dic['type'], features_dic['uri'], features_dic['track_href'], features_dic['analysis_url']
...
def write_tsv(data, is_first):
    ...
    keys = data[0].keys()
    if is_first:
        with open('songs.tsv', 'w', newline='', ) as output_file:
            dict_writer = csv.DictWriter(output_file, keys, delimiter='\t')
            dict_writer.writeheader()
            dict_writer.writerows(data)
    else:
        with open('songs.tsv', 'a', newline='', ) as output_file:
            dict_writer = csv.DictWriter(output_file, keys, delimiter='\t')
            dict_writer.writerows(data)
...
if __name__ == '__main__':
    spotify = connect_spotify('../config.json')
    parsed_data_arr = parse_json('../data/mpd.slice.0-999.json')
    FROM = 60000
    TO = 6300
    print(f'max: {len(parsed_data_arr)}')
    generate_data_new_way(spotify, parsed_data_arr[FROM:TO])
    insert_csv_into_db('../config.json', 'music1', 'songs.tsv')

    # generate_data_new_way(spotify, parsed_data_arr[FROM:TO])
    # # preprocess_playlist_1(parsed_data_arr[:300], spotify)
    # # generate_data(spotify, parsed_data_arr[:20])
    # insert_csv_into_db('config.json', 'music', 'songs.tsv')
